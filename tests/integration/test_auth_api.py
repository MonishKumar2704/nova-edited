...
def test_full_connect_callback_status_disconnect_flow(app, client):
    ...
    callback_resp = client.get(f"/api/v1/auth/google/callback?code=abc&state={state}&mode=json")
...
def test_callback_with_bad_state_returns_authentication_error(app, client):
    ...
    resp = client.get("/api/v1/auth/google/callback?code=abc&state=not-a-real-state&mode=json")
...
def test_callback_with_provider_error_returns_authentication_error(app, client):
    _install_configured_service(app)
    resp = client.get("/api/v1/auth/google/callback?error=access_denied&mode=json")
    assert resp.status_code == 401


def test_callback_default_mode_redirects_back_to_nova_on_success(app, client):
    # Task: Google's own redirect lands the browser here directly (no
    # frontend JS involved in this hop) - the default response must be a
    # same-tab redirect back to the NOVA UI, not a bare JSON page.
    oauth_client = _install_configured_service(app)

    connect_resp = client.get("/api/v1/auth/google/connect?mode=json")
    state = connect_resp.get_json()["authorization_url"]
    _, kwargs = oauth_client.build_authorization_url.call_args
    state = kwargs["state"]
    ...
    assert callback_resp.headers["Location"] == "/?google=connected"

    status_resp = client.get("/api/v1/auth/google/status")
    assert status_resp.get_json()["connected"] is True


def test_callback_default_mode_redirects_back_to_nova_on_error(app, client):
    _install_configured_service(app)
    resp = client.get("/api/v1/auth/google/callback?error=access_denied")
    assert resp.status_code == 302
    assert resp.headers["Location"].startswith("/?google=error&reason=")
