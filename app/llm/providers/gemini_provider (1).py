                    ...
                    "functionDeclarations": [
                        {
                            ...
                            "parameters": _sanitize_schema(_convert_input_schema(tool.get("input_schema"))),
                        }
                        for tool in tools
                    ]
                }
...
def _convert_input_schema(schema: dict[str, Any] | None) -> dict[str, Any]:
    """Convert a Nova `Tool.input_schema` into a Gemini `Schema` object.

    `Tool.input_schema` (see `app.tools.base.Tool`) is a flat mapping of
    ``{field_name: {"type": ..., "required": bool, ...}}`` - convenient for
    our own argument validation, but *not* the shape Gemini's
    `functionDeclarations[].parameters` field (a `Schema` proto) expects.
    Gemini wants `{"type": "object", "properties": {...}, "required": [...]}`,
    with per-field `required` flags pulled out into that top-level array.

    Passed straight through, every field name (`"to"`, `"subject"`, ...)
    is read as an unrecognized property of the `Schema` object itself,
    which is exactly the `Unknown name "..." ... Cannot find field` /
    `Starting an object on a scalar field` errors Gemini returns - the
    latter happens whenever a tool happens to have a field literally named
    `description` (a real `Schema` field expecting a string) whose value is
    our nested field-spec object instead.
    """
    if not schema:
        return {"type": "object", "properties": {}}

    # Already a proper JSON-Schema/Gemini Schema object (e.g. a tool that
    # reuses one of the `ai.*` response schemas) - nothing to convert.
    ...
    for field_name, field_spec in schema.items():
        ...
        if field_spec.pop("required", False):
            required.append(field_name)
        if field_spec.get("type") == "array":
            items = field_spec.get("items")
            if isinstance(items, str):
                # Our schemas sometimes shorthand `"items": "string"`
                # instead of a full `{"type": "string"}` Schema object.
                field_spec["items"] = {"type": items}
            elif not isinstance(items, dict):
                # Gemini's Schema requires `items` on every array field;
                # some of our schemas omit it entirely (e.g. `tags`,
                # `video_ids`, `label_ids`). Every such field in this
                # codebase is a list of ID/string values, so default to
                # that rather than rejecting the whole tool.
                field_spec["items"] = {"type": "string"}
        properties[field_name] = field_spec

    converted: dict[str, Any] = {"type": "object", "properties": properties}
    if required:
